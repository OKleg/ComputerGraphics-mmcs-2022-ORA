    ...
    public class Dodecahedron : Polyhedron
    {
        ...
        public Dodecahedron() : base()
        {
          ...
          // 6 - 15,10,8,14,7,1,3,19,18
            ...
            this.edges.AddRange(new Face(new List<int>() { 0, 1, 14, 8, 12 }).GetEdges());// 0 - 1,12,16
            this.edges.AddRange(new Face(new List<int>(){ 16, 9, 18, 1, 0 }).GetEdges());// 1 - 0,14,18
            this.edges.AddRange(new Face(new List<int>(){ 0, 12, 4, 5, 16 }).GetEdges());
            this.edges.AddRange(new Face(new List<int>(){ 12, 8, 10, 13, 4 }).GetEdges());
            this.edges.AddRange(new Face(new List<int>(){ 6, 15, 10, 8, 14 }).GetEdges());// 4 - 5,12,13
            this.edges.AddRange(new Face(new List<int>(){ 7, 6, 14, 1, 18 }).GetEdges());// 6 - 7,14,15
            this.edges.AddRange(new Face(new List<int>(){ 9, 11, 19, 7, 18 }).GetEdges());
            this.edges.AddRange(new Face(new List<int>(){ 9, 16, 5, 17, 11 }).GetEdges());
            this.edges.AddRange(new Face(new List<int>(){ 19, 3, 15, 6, 7 }).GetEdges());
            this.edges.AddRange(new Face(new List<int>(){ 13, 2, 17, 5, 4 }).GetEdges());//12 - 0,4,8
            this.edges.AddRange(new Face(new List<int>(){ 15, 3, 2, 13, 10 }).GetEdges());
            this.edges.AddRange(new Face(new List<int>() { 2, 3, 19, 11, 17 }).GetEdges());//18 - 1,7,9
      //  };
